...
export const ensureProfileRecord = async (user) => {
  ...
  // How accounts that predate the directory get into it: this runs on every
  // provider sign-in, so there is no backfill to remember to run
  await syncDirectoryEntry(id);
